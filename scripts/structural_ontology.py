#!/usr/bin/env python3
"""Generate / staleness-check isnad-ingest-platform's structural ontology index (#116).

This is the isnad-ingest-platform (consumer) side of the C×T2 distributed
structural ontology (parent meta noorinalabs-main#820, generator
noorinalabs-main#855; pilot isnad-graph#1128). It does NOT contain the
extraction logic — that lives, single-source-of-truth, in the OWNED generator
package ``ontology_gen`` in **noorinalabs-main**
...
fork: a fix to the extractor in noorinalabs-main would silently not reach the
...
* **CI** checks out noorinalabs-main as a sibling (resolving the ref to the
  ...
  pattern, cf. ``.github/workflows/structural-ontology.yml`` /
  feedback_cross_repo_wave_ref_resolution, deploy#159) and points this script
  at it via ``--gen-lib`` / ``ONTOLOGY_GEN_LIB``.
* **Local dev** relies on the standard org layout (child repos cloned beneath
  ``noorinalabs-main/``); :func:`locate_generator` walks up to find the
  parent's ``.claude/lib/ontology_gen`` automatically. ``ONTOLOGY_GEN_LIB``
  overrides for any non-standard checkout.

Tradeoff: the committed index is coupled to the generator VERSION in
noorinalabs-main. A generator change there can turn a consumer's staleness gate
red until the index is regenerated and re-committed — expected for a drift gate,
but the fan-out should sequence generator changes ahead of a coordinated
consumer-repo index refresh. See ``docs/devops/ontology-structural.md``.

Subcommands
===========
* ``emit``  — (re)generate the committed index in place.
* ``check`` — regenerate into a temp dir and fail (exit 1) if it differs from
  the committed index. This is what the CI job + the pre-commit hook run.
* ``register-merge-driver`` — register the union merge-driver in git config
  (the ``.gitattributes`` line is committed; the driver registration is
  per-clone local state, so ``make setup-hooks`` runs this). NOTE: the driver
  is invoked in PYTHON MODULE form (``python3 -m ontology_gen.merge_driver``),
  NOT as a bare script — ``merge_driver.py`` uses a package-relative import
  (``from .model``), so a plain ``python3 .../merge_driver.py`` invocation
  raises ImportError.
"""
...
REPO_NAME = "noorinalabs-isnad-ingest-platform"
...
def locate_generator(repo_root: Path, explicit: str | None) -> Path | None:
    ...
    graceful local skip (a developer who hasn't cloned noorinalabs-main, or
    whose parent checkout is on a branch that predates the generator).
