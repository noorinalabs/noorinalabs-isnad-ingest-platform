#!/usr/bin/env bash
# Apply the noorinalabs-isnad-ingest-platform default-branch protection ruleset.
#
...
#   * Required status checks: ingest-platform has unconditional PR CI (ci.yml has
#     no paths: filter), so the ruleset hard-requires its gate contexts (job
#     NAMES, not workflow names): `lint-and-typecheck`, `security-audit`, `test`,
#     and `precommit-ci-sync`. The docs.yml jobs are path-filtered and so are
#     NOT required (requiring a check that does not run on code-only PRs would
#     block every such merge). Confirm the contexts against live check-runs at
...
#
# Usage:
#   ./apply-ruleset.sh                 # apply to noorinalabs/noorinalabs-isnad-ingest-platform
...
REPO="${REPO:-noorinalabs/noorinalabs-isnad-ingest-platform}"
